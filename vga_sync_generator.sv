/*
 * File: vga_sync_generator.sv
 * Author: Harlan J. Waldrop <harlan@ieee.org>
 *
 * Autogenerated on 2019-04-02 13:45:11 by mksv
 */
 ...
 * DAC Blanking Signal
 */
always_ff @(posedge clk or posedge reset) begin : proc_vga_blank
	if (reset)
		blank_n <= 0;
	else
 ...
 */
always_ff @(posedge clk or posedge reset) begin : proc_vga_h
	if (reset) begin
		h_count <= 1'b0;
		next_x  <= 1'b0;
		h_sync  <= 1'b1;
 ...
 */
always_ff @(posedge clk or posedge reset) begin : proc_vga_v
	if (reset) begin
		v_count <= 1'b0;
		next_y  <= 1'b0;
		v_sync  <= 1'b1;
